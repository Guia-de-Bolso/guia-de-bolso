...
BEGIN
  ...
  IF v_used >= v_limit THEN
    RETURN jsonb_build_object(
      'allowed', false,
      'code', 'LIMIT_REACHED',
      'message', 'Você usou suas ' || v_limit || ' buscas com IA de hoje. O limite reinicia à meia-noite. Assine o Premium por R$9,90/mês para uso ilimitado.',
      'usage', jsonb_build_object(
        'premium', false,
        'day', v_day,
        'month', v_day,
...
BEGIN
  ...
  IF NOT FOUND THEN
    ...
    RETURN jsonb_build_object(
      ...
      'usage', jsonb_build_object(
        ...
        'month', v_day,
        'resets_at', v_resets_at,
        'buscas', jsonb_build_object('used', 0, 'limit', v_limit_busca, 'remaining', v_limit_busca),
  ...
  IF v_perfil.premium_ativo IS TRUE
    ...
    RETURN jsonb_build_object('allowed', true, 'code', 'OK', 'usage', jsonb_build_object(
      ...
      'month', v_day,
      'resets_at', v_resets_at,
      'buscas', jsonb_build_object('used', COALESCE(v_perfil.buscas_ia, 0), 'limit', v_limit_busca, 'remaining', null),
  ...
  IF v_used >= v_limit THEN
    RETURN jsonb_build_object(
      ...
      'usage', jsonb_build_object(
        ...
        'month', v_day,
        'resets_at', v_resets_at,
        'buscas', jsonb_build_object('used', v_buscas, 'limit', v_limit_busca, 'remaining', greatest(0, v_limit_busca - v_buscas)),
