...
export function fetchAvaliacoesLugar(supabase, lugarId) {
  // Sem embed em `perfis`: não há FK avaliacoes.user_id → perfis no PostgREST
  // (PGRST200). Nome/foto vêm do snapshot `autor_nome` / `autor_foto_url`.
  return supabase
    .from("avaliacoes")
    .select(
      "id, lugar_id, user_id, nota, comentario, aspectos, status, created_at, autor_nome, autor_foto_url, resposta_estabelecimento"
    )
    .eq("lugar_id", lugarId)
