...
test("isPublicSeoPath", () => {
  ...
  assert.equal(isPublicSeoPath("/roteiros/abc"), true);
  assert.equal(isPublicSeoPath("/roteiros"), false);
  assert.equal(isPublicSeoPath("/guia/o-que-fazer-em-imbituba"), true);
  assert.equal(isPublicSeoPath("/q/praia-da-vila"), true);
...
test("isPublicMarketingPath", () => {
  ...
  assert.equal(isPublicMarketingPath("/roteiros"), true);
  assert.equal(isPublicMarketingPath("/atrativos"), true);
  assert.equal(isPublicMarketingPath("/lugares/foo"), true);
  assert.equal(isPublicMarketingPath("/q/foo"), true);
...
test("getMarketingRouteAction", () => {
  ...
  assert.equal(getMarketingRouteAction("/roteiros"), "continue");
  assert.equal(getMarketingRouteAction("/atrativos"), "continue");
  assert.equal(getMarketingRouteAction("/lugares/praia-da-vila"), "continue");
  assert.equal(getMarketingRouteAction("/q/praia-da-vila"), "continue");
  assert.equal(getMarketingRouteAction("/.well-known/assetlinks.json"), "continue");
