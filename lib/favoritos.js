...
export async function toggleFavoritoLugar(supabase, user, lugar, setFavoritoIds) {
  ...
  try {
    ...
    if (jaFavorito) {
      ...
      await registrarLog(supabase, user, "desfavoritou", buildLugarLogDetalhes(lugar));
      return "removed";
    }
    ...
    await cacheLugarFavoritoFromServer(supabase, user.id, lugarId);

    await registrarLog(supabase, user, "favoritou", buildLugarLogDetalhes(lugar));
    return "added";
  } finally {
