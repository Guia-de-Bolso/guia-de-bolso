import { cache } from "react";
import {
  normalizeFotosLegado,
  normalizeLugarForClient,
  normalizeTagsFromJoin,
  resolveLugarFotosIniciais,
} from "./lugarPageDataNormalize.js";
import { fetchLugarSeoBundleCached } from "./lugarSeoData.js";
import { createPublicPageServerClient } from "./supabase/pageServer.js";

export {
  normalizeFotosLegado,
  normalizeLugarForClient,
  normalizeTagsFromJoin,
  resolveLugarFotosIniciais,
} from "./lugarPageDataNormalize.js";

/**
 * Dados iniciais do detalhe do lugar para SSR + hidratação imediata no client.
 * @param {import('@supabase/supabase-js').SupabaseClient|string} supabaseOrRouteParam
 * @param {string} [routeParam]
 * @returns {Promise<{
 *   lugar: object|null,
 *   localizacao: object|null,
 *   rating: { media: number, count: number }|null,
 *   tags: object[],
 *   fotos: string[],
 *   error: import('@supabase/supabase-js').PostgrestError|null,
 * }>}
 */
export async function fetchLugarPageInitialData(supabaseOrRouteParam, routeParam) {
  if (typeof supabaseOrRouteParam === "string") {
    return fetchLugarPageInitialDataCached(supabaseOrRouteParam);
  }

  return fetchLugarPageInitialDataWithClient(supabaseOrRouteParam, routeParam);
}

/**
 * @param {import('@supabase/supabase-js').SupabaseClient} supabase
 * @param {string} routeParam
 */
async function fetchLugarPageInitialDataWithClient(supabase, routeParam) {
  const bundle = await fetchLugarSeoBundleCached(routeParam);

  if (bundle.error || !bundle.lugar) {
    return {
      lugar: null,
      localizacao: null,
      rating: null,
      tags: [],
      fotos: [],
      error: bundle.error,
    };
  }

  const lugarId = bundle.lugar.id;
  const [tagsRes, fotosLegadoRes] = await Promise.all([
    supabase.from("lugares_tags").select("tags(*)").eq("lugar_id", lugarId),
    supabase.from("fotos_lugar").select("*").eq("lugar_id", lugarId),
  ]);

  const fotosLegado = normalizeFotosLegado(fotosLegadoRes.data);
  const lugar = normalizeLugarForClient(bundle.lugar);

  return {
    lugar,
    localizacao: bundle.localizacao,
    rating: bundle.rating,
    tags: normalizeTagsFromJoin(tagsRes.data),
    fotos: resolveLugarFotosIniciais(lugar, fotosLegado),
    error: null,
  };
}

/**
 * Versão cacheada por request (slug).
 * @param {string} routeParam
 */
export const fetchLugarPageInitialDataCached = cache(
  async function fetchLugarPageInitialDataCached(routeParam) {
    const supabase = await createPublicPageServerClient();
    if (!supabase) {
      return {
        lugar: null,
        localizacao: null,
        rating: null,
        tags: [],
        fotos: [],
        error: null,
      };
    }

    return fetchLugarPageInitialDataWithClient(supabase, routeParam);
  }
);
