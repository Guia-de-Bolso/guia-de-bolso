...
export const LUGAR_SELECT_LIST =
  "id, nome, slug, descricao, categoria, subcategoria, imagem_url, fotos, status, eh_parceiro, parceiro_inicio_em, conteudo_curadoria, horarios, mostrar_horarios, created_at, localizacoes(latitude, longitude, endereco_completo), lugares_tags(tags(id, nome, icone))";
...
export async function queryAllLugaresAtivos(
  ...
  const fetchPage = async (pageSelect, offset, limit) => {
    let query = applyPublicLugarFilters(
      supabase.from("lugares").select(pageSelect)
