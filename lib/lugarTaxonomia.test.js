...
const escolaSurf = { categoria: "Natureza", subcategoria: "Surf" };
assert.equal(getEffectiveCategoria(escolaSurf), "Aventura");
assert.equal(normalizeLugarTaxonomia(escolaSurf).categoria, "Aventura");
...
for (const lugar of fixtures) {
  for (const categoria of ["Natureza", "Noite", "Gastronomia", "Cultura", "Aventura"]) {
