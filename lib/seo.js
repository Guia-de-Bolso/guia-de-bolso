...
export function buildAtrativoMetadata(rota) {
  ...
  const title = `${nome}${categoria} · Atrativo em Imbituba`;
  const fotos = getFotosFromAtrativo(rota);

  return buildPageMetadata({
    title,
    description: rota?.descricao || `Atrativo curado ${nome} no Guia de Bolso.`,
    path: `/atrativos/${rota.id}`,
    image: fotos[0],
  });
}
...
export function buildLandingMetadata() {
  return buildPageMetadata({
    ...
    description:
      "O que fazer em Imbituba: praias, Praia do Rosa, restaurantes e atrativos curados. App de turismo local — Guia de Bolso Imbituba.",
    path: "/",
  });
}
...
export function buildSobreMetadata() {
  return buildPageMetadata({
    ...
    description:
      "Conheça o Guia de Bolso Imbituba: app e site de turismo em Imbituba, SC. Sem relação com Guiabolso (finanças). Praias, atrativos e lugares curados.",
    path: "/sobre",
  });
}
