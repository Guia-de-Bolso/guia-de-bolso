 ...
 *
 * - presenca: perfil completo permanente sem destaque pago — utilitários (farmácias,
 *   mercados, saúde) e patrimônio cultural (igrejas, museus, monumentos).
 *   sem destaque pago (carrossel, badge, IA, QR, relatório).
 * - lancamento: perfil completo gratuito até `perfil_promo_ate` (experiências).
...
export const PLANO_TIER_LABELS = {
  ...
  [PLANO_TIER.PRESENCA]: "Presença (utilitário)",
  [PLANO_TIER.LANCAMENTO]: "Lançamento (completo grátis)",
  [PLANO_TIER.PARCEIRO]: "Parceiro (pago)",
};

/** Subcategorias Presença permanente (perfil completo grátis, sem destaque pago). */
...
/**
 * Estabelecimento comercial que deve ficar no plano Presença (utilitário, sem promo de Lançamento).
...
/**
 * Promo Lançamento: perfil completo temporário sem ser parceiro pago.
 * Utilitário/Presença não entra aqui (é permanente, outra regra).
 * @param {object|null|undefined} lugar
 * @param {string} [hoje]
 * @returns {boolean}
 */
export function hasPerfilCompletoGratuito(lugar, hoje = hojeISO()) {
  if (!lugar || !isLugarEstabelecimento(lugar)) return false;
  if (isParceiro(lugar)) return false;
  if (isSubcategoriaPresenca(lugar.subcategoria)) return false;
  return isPerfilPromoAtivo(lugar.perfil_promo_ate, hoje);
}

/**
 * Perfil completo visível no app público (galeria, links, avaliações, história).
 * Locais públicos, curadoria, Parceiro, utilitário/Presença e promo Lançamento.
 * @param {object|null|undefined} lugar
 * @param {string} [hoje]
 * @returns {boolean}
 */
export function hasPerfilCompletoPublico(lugar, hoje = hojeISO()) {
  if (!lugar) return false;
  if (!isLugarEstabelecimento(lugar)) return true;
  if (isParceiro(lugar)) return true;
  if (isConteudoCuradoria(lugar)) return true;
  if (isSubcategoriaPresenca(lugar.subcategoria)) return true;
  return isPerfilPromoAtivo(lugar.perfil_promo_ate, hoje);
}

/**
 * Estabelecimento no tier Presença (não parceiro, sem promo Lançamento).
 * Utilitários entram aqui mesmo com perfil completo no app.
 * @param {object|null|undefined} lugar
 * @param {string} [hoje]
 * @returns {boolean}
 */
