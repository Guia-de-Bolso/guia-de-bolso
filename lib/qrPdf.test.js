...
assert.equal(QR_PDF_COPY.appDomain, "app.guiadebolso.app");

const adesivoDoc = createQrPdfDocument("adesivo");
assert.equal(Math.round(adesivoDoc.internal.pageSize.getWidth()), 80);
assert.equal(Math.round(adesivoDoc.internal.pageSize.getHeight()), 80);

// Formato desconhecido cai no padrão (adesivo).
const fallbackDoc = createQrPdfDocument("mesa");
assert.equal(Math.round(fallbackDoc.internal.pageSize.getWidth()), 80);
