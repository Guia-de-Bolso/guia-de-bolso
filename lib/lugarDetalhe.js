...
const FRASES_POR_SUBCATEGORIA = {
  ...
  Monumentos: [
    "Marco histórico que vale a visita 🏛️",
    "Parte da memória e da identidade da cidade 📜",
  ],
  "Igrejas e templos": [
    "Espaço de fé e história local ⛪",
  ],
};
...
const CONTEXTO_KEYWORDS = [
  {
    match: /\bmuseu|\bpatrim[oô]nio|\bmonumento|\bmemorial\b/i,
    frase: "Patrimônio histórico para conhecer de perto 🏛️",
  },
  {
  ...
  {
    // Só ofício (eletricista) — não “usina elétrica”, museu, etc.
    match: /\beletricistas?\b|servi[cç]os?\s+el[eé]tric/i,
...
const TAG_KEYWORDS = [
  ...
  { match: /famíl|crianç|kids/i, frase: "Ideal para famílias com crianças 👨‍👩‍👧‍👦" },
  {
    ...
    categories: ["Gastronomia", "Natureza", "Bem-estar", "Cultura"],
  },
...
function getDuracaoExperienciaLocal(lugar) {
  const sub = (lugar?.subcategoria || "").toLowerCase();
  if (/praia/.test(sub)) return "2–4h";
  if (/trilha/.test(sub)) return "1–2h";
  if (/cachoeira|lagoa/.test(sub)) return "1–2h";
  if (/mirante/.test(sub)) return "30–45 min";
  if (lugar?.categoria === "Natureza") return "1–2h";
  if (lugar?.categoria === "Aventura") return "2–3h";
...
function getMelhorHorarioVisita(lugar, tags = []) {
  ...
  if (/praia/.test(sub)) return "Manhã ou fim de tarde";
  if (/trilha|cachoeira/.test(sub)) return "Pela manhã";
  if (lugar?.categoria === "Natureza") return "Com luz do dia";
  return "Com calma";
}
...
function parseInfoAcao(id, emoji, label) {
  ...
  if (id === "duracao") {
    const match = label.match(
      /(\d+\s*[–-]\s*\d+\s*(?:h|min)|~\d+\s*[–-]\s*\d+\s*(?:h|min)|\d+[–-]\d+h|~\d+[–-]\d+h|\d+h|~\d+h|\d+\s*min)/i
    );
    const valor = match?.[1]?.replace(/\s+/g, " ").trim() || label;
    return infoAcao(id, emoji, valor, "Tempo médio", label);
  }
  ...
  if (id === "horario") {
    return infoAcao(id, "🌅", label, "Melhor horário", label);
  }
...
export function getAcoesRapidasLocais(lugar, tags = [], distancia) {
  ...
  acoes.push(parseInfoAcao("horario", "🌅", getMelhorHorarioVisita(lugar, tags)));
 ...
 * Cardápio só entra em Gastronomia; nas demais categorias o slot equivalente no teaser é Facebook.
 * @param {{ telefone?: string, instagramHref?: string, facebookHref?: string, cardapioUrl?: string, siteUrl?: string, categoria?: string, subcategoria?: string }} params
...
export function getAcoesRapidasEstabelecimento({
  ...
  const usaCardapio = categoriaUsaCardapio({ categoria, subcategoria });
...
/**
 * Cardápio (mesmo bloqueado) só faz sentido em Gastronomia.
 * @param {{ categoria?: string, subcategoria?: string }|null|undefined} lugar
 * @returns {boolean}
 */
export function categoriaUsaCardapio(lugar) {
  return getEffectiveCategoria(lugar) === "Gastronomia";
}

/**
 * Ações do plano Presença: WhatsApp/telefone liberado; links premium como teaser bloqueado.
 * @param {{ telefone?: string, categoria?: string, subcategoria?: string }} params
 * @returns {Array<{ id: string, label: string, href: string|null }>}
 */
export function getAcoesRapidasPresenca({ telefone, categoria, subcategoria }) {
  const usaCardapio = categoriaUsaCardapio({ categoria, subcategoria });
  const mid = usaCardapio
    ? { id: "cardapio", label: "Cardápio", href: null }
  ...
  return [
    ...
    mid,
    { id: "site", label: "Site", href: null },
  ];
}

/**
 * Ações teaser (sem link) — legado; prefira {@link getAcoesRapidasPresenca}.
 * @param {{ categoria?: string, subcategoria?: string }|null|undefined} [lugar]
 * @returns {Array<{ id: string, label: string, href: null }>}
 */
export function getAcoesRapidasBloqueadas(lugar) {
  const mid = categoriaUsaCardapio(lugar)
    ? { id: "cardapio", label: "Cardápio", href: null }
    : { id: "facebook", label: "Facebook", href: null };
