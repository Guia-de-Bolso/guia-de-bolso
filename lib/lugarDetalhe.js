...
const FRASES_POR_CATEGORIA = {
  ...
  Serviços: [
    "Serviço de qualidade perto de você ✨",
...
const FRASES_POR_SUBCATEGORIA = {
  ...
  Saúde: [
    "Atendimento de saúde perto de você quando precisar 🏥",
  ],
  Farmácias: [
...
export function getFraseConvencimento(lugar, tags = []) {
  ...
  if (subcategoria && FRASES_POR_SUBCATEGORIA[subcategoria]) {
    return pickFraseDoPool(
      FRASES_POR_SUBCATEGORIA[subcategoria],
      lugar.id ?? lugar.nome
    );
  }
  ...
  for (const tag of tags) {
    ...
    for (const { match, frase, outdoorOnly } of TAG_KEYWORDS) {
  ...
  return pickFraseDoPool(pool, lugar.id ?? lugar.nome);
}
