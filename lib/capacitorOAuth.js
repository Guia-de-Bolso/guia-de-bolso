...
/**
 * OAuth Google via redirect no WebView (mesmo domínio do app Capacitor).
...
/**
 * Inicia login Google.
 * - iOS Capacitor (padrão): OAuth web no WebView — sem crash do SDK nativo.
 * - iOS Capacitor (NEXT_PUBLIC_IOS_GOOGLE_NATIVE=true): Sign-In nativo + id_token.
...
export async function signInWithGoogleOAuth(supabase, postLoginPath = "/") {
  if (isNativeCapacitorApp()) {
    if (isNativeIosApp() && !shouldUseIosNativeGoogleSignIn()) {
      return signInWithGoogleWebOAuth(supabase, postLoginPath);
    }

    if (!canUseNativeGoogleSignIn()) {
      const missingIosClient = isNativeIosApp();
      return {
        error: new Error(
          missingIosClient
