 ...
 * @param {import("react").RefObject<HTMLDivElement | null>} carouselRef
 * @param {number} [slideCount=0]
 * @param {{ indexThreshold?: number, maxDeltaPerGesture?: number, initialIndex?: number }} [options]
 * @returns {number}
 */
export function useControlledPhotoCarousel(carouselRef, slideCount = 0, options = {}) {
  ...
  useEffect(() => {
    ...
    const start = Math.min(Math.max(0, initialIndex), maxIndex);
    setIndex(start);
    const element = carouselRef.current;
    if (element) scrollCarouselToIndex(element, start);
  }, [slideCount, carouselRef, initialIndex]);
