 ...
 * @returns {string}
 */
function getExpectedGoogleTokenAudience() {
  if (Capacitor.getPlatform() === "ios") {
...
async function loginWithGoogleNative(retry = false, useNonce = true) {
  ...
  try {
    ...
    const expectedAud = getExpectedGoogleTokenAudience();
    const decoded = decodeJwtPayload(result.idToken);
    if (decoded?.aud && expectedAud && decoded.aud !== expectedAud) {
      throw new Error(
        `Client ID inválido no token (${decoded.aud}). Confira os Client IDs na Vercel e no Supabase.`
      );
    }
