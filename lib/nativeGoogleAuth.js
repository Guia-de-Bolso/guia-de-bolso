...
import {
  ANDROID_GOOGLE_SHA1_HINT,
  GOOGLE_LOGIN_TIMEOUT_MS,
  mapAndroidGoogleCancelToError,
  withGoogleLoginTimeout,
} from "@/lib/googleLoginNativeHelpers";

const ANDROID_SHA1_HINT = ANDROID_GOOGLE_SHA1_HINT;
...
export async function signInWithGoogleNative(supabase, postLoginPath = "/") {
  ...
  try {
    ...
    const response = await withGoogleLoginTimeout(
      SocialLogin.login({
        provider: "google",
        options: {},
      }),
      GOOGLE_LOGIN_TIMEOUT_MS,
      "O Google não respondeu após escolher a conta. Feche o app, abra de novo e tente outra vez."
    );
    ...
    const androidCancelError = mapAndroidGoogleCancelToError(
      error,
      Capacitor.getPlatform()
    );
    if (androidCancelError) {
      return { error: androidCancelError };
    }
  ...
  const user =
    signInData?.user ?? (await getSessionUser(supabase));

  if (!user) {
    return {
      error: new Error(
        "Login concluído, mas a sessão não foi salva. Feche o app e tente novamente."
      ),
    };
  }
  ...
  void (async () => {
    try {
      ...
      console.warn("signInWithGoogleNative: pós-login", setupError);
    }
  })();
