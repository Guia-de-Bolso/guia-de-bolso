...
async function loginWithGoogleNative(retry = false, useNonce = true) {
  ...
  const preferNonce = platform !== "ios" && useNonce;
  const { loginOptions, rawNonce, nonceDigest } = await buildGoogleLoginOptions(preferNonce);
  ...
  try {
    ...
    if (preferNonce && nonceDigest && decoded?.nonce && decoded.nonce !== nonceDigest) {
      throw new Error("Nonce do token não confere. Tente novamente.");
    }

    return { result, rawNonce, decoded, usedNonce: preferNonce };
  } catch (error) {
