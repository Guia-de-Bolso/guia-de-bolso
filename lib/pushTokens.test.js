...
import { disableInvalidPushTokens, disablePushTokens, getEnabledPushTokensForUsers, upsertPushToken } from "./pushTokens.js";
...
test("disableInvalidPushTokens desativa lista de tokens", async () => {
  const admin = {
    from() {
      return {
        update() {
  ...
  const disabled = await disableInvalidPushTokens(admin, ["a", "b"]);
