...
import {
  ...
  buildTopLugaresPorReceita,
  formatFilaAbordagemCsv,
  isLugarPotencialReceita,
} from "./adminKpis.js";
import {
...
assert.equal(hasPerfilCompletoPublico(farmacia, "2026-08-31"), true);
assert.equal(hasPerfilCompletoPublico(praia, "2026-08-31"), true);
assert.equal(
  hasPerfilCompletoPublico(
    {
      nome: "Skatepark",
...
assert.equal(
  hasPerfilCompletoPublico(
    { ...estabelecimento, perfil_promo_ate: null },
    "2026-08-31"
  ),
  false
);
...
assert.equal(isSubcategoriaPresenca("Saúde"), true);
assert.equal(isSubcategoriaPresenca("Igrejas e templos"), true);
assert.equal(isSubcategoriaPresenca("Museus"), true);
assert.equal(isSubcategoriaPresenca("Monumentos"), true);
...
assert.equal(
  shouldLugarDefaultPresenca({
    nome: "Capela",
    categoria: "Cultura",
    subcategoria: "Igrejas e templos",
    eh_parceiro: false,
  }),
  true
);
...
assert.equal(isLugarPotencialReceita({ categoria: "Gastronomia" }), true);
assert.equal(isLugarPotencialReceita({ categoria: "Natureza", subcategoria: "Praias" }), false);
assert.equal(
  isLugarPotencialReceita({
    categoria: "Gastronomia",
    conteudo_curadoria: true,
...
const topPorReceita = buildTopLugaresPorReceita(
  [
    { acao: "visualizou_lugar", detalhes: { lugar_id: "1" } },
    { acao: "visualizou_lugar", detalhes: { lugar_id: "1" } },
    { acao: "ir_agora", detalhes: { lugar_id: "1" } },
    { acao: "visualizou_lugar", detalhes: { lugar_id: "3" } },
    { acao: "visualizou_lugar", detalhes: { lugar_id: "3" } },
    { acao: "visualizou_lugar", detalhes: { lugar_id: "3" } },
    { acao: "ir_agora", detalhes: { lugar_id: "4" } },
  ],
  new Map([
    ["1", { nome: "Bar", categoria: "Gastronomia" }],
    ["3", { nome: "Praia da Vila", categoria: "Natureza", subcategoria: "Praias" }],
    ["4", { nome: "Mirante curado", categoria: "Cultura", conteudo_curadoria: true }],
  ])
);
assert.equal(topPorReceita.comerciais[0].nome, "Bar");
assert.equal(topPorReceita.curadoria[0].nome, "Praia da Vila");
assert.equal(topPorReceita.curadoria[1].nome, "Mirante curado");
assert.equal(topPorReceita.comerciais.length, 1);
assert.equal(topPorReceita.curadoria.length, 2);
