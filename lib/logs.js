...
export function formatarAcaoLog(log) {
  ...
  const labels = {
    ...
    claim_perfil: `Pediu desbloqueio de ${detalhes.lugar_nome || "um local"}`,
    ...
    admin_excluiu_avaliacao: `Excluiu avaliação de ${detalhes.autor_nome || "usuário"} em ${detalhes.lugar_nome || "um lugar"}`,
    admin_excluiu_feedback: `Excluiu feedback de ${detalhes.autor_nome || "usuário"}`,
...
export function getLogAcaoBadge(acao) {
  const badges = {
    ...
    claim_perfil: { label: "Claim perfil", className: "bg-amber-100 text-amber-800" },
    ...
    admin_excluiu_avaliacao: {
      label: "Admin excluiu avaliação",
      className: "bg-red-100 text-red-800",
    },
    admin_excluiu_feedback: {
      label: "Admin excluiu feedback",
      className: "bg-red-100 text-red-800",
    },
...
export function formatarDetalhesLog(detalhes = {}) {
  ...
  if (detalhes.rota_id) return `Roteiro #${detalhes.rota_id}`;
