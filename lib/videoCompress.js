...
/**
 * Vídeo pode ir ao Storage sem transcodificar (dentro dos limites de saída).
 * @param {File} file
 * @param {{ width?: number, height?: number }} [metadata]
 * @returns {boolean}
 */
export function canUploadVideoAsIs(file, metadata = {}) {
  const { width = 0, height = 0 } = metadata;

  if (file.size > LUGAR_VIDEO_LIMITS.maxOutputBytes) return false;
...
export function needsVideoCompression(file, metadata = {}) {
  if (!canUploadVideoAsIs(file, metadata)) return true;
...
export async function compressVideoFile(file, options = {}) {
  ...
  try {
    ...
    if (canUploadVideoAsIs(file, metadata)) {
      onProgress?.(100);
      return file;
    }

    throw new Error(
      "Não foi possível otimizar o vídeo neste navegador. Envie um MP4 de até 25 MB ou tente no Chrome no computador."
    );
  }
