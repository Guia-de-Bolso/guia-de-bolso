 ...
 */
export function toGalleryPhoto(entry) {
 ...
 */
export function toGalleryPhotos(entries) {
  return (entries ?? []).map(toGalleryPhoto).filter(Boolean);
}
 ...
 * @returns {GalleryPhoto[]}
 */
export function toGalleryPhotosFromUrls(urls) {
 ...
 * @returns {GalleryPhoto[]}
 */
export function getGalleryPhotosFromLugar(lugar) {
  ...
  if (lugar?.imagem_url) return [{ url: lugar.imagem_url }];
  return [];
}
...
/**
 * Seleciona até três fotos reais para o preview bloqueado do perfil básico.
 * A capa aparece primeiro quando está disponível.
 * @param {GalleryPhoto[]} photos
 * @param {string} capaUrl
 * @param {number} [limit=3]
 * @returns {GalleryPhoto[]}
 */
export function getLockedGalleryPreview(photos, capaUrl, limit = 3) {
  if (limit <= 0) return [];
  if (!photos?.length) return capaUrl ? [{ url: capaUrl }] : [];

  const capa = capaUrl
    ? photos.find((photo) => photo.url === capaUrl) ?? { url: capaUrl }
    : photos[0];
  const restantes = photos.filter((photo) => photo.url !== capa.url);

  return [capa, ...restantes].slice(0, limit);
}
