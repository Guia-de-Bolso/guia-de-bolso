...
export const ALERTA_TIPO = {
  ...
  PARCEIRO_SEM_CONTRATO_ATIVO: "parceiro_sem_contrato_ativo",
};
...
export const ALERTA_ICONE = {
  ...
  [ALERTA_TIPO.PARCEIRO_SEM_CONTRATO_ATIVO]: "📋",
};
...
export async function fetchAdminAlertas(supabase) {
  ...
  if (!contratosRes.error) {
    ...
    const contratosPorLugar = Object.fromEntries(
      contratosAtivos.map((item) => [item.lugar_id, item])
    );
    ...
    for (const lugar of parceirosRes.error ? [] : parceirosRes.data ?? []) {
      if (!deveAlertarParceiroSemContratoAtivo(lugar, contratosPorLugar)) continue;

      alertas.push({
        tipo: ALERTA_TIPO.PARCEIRO_SEM_CONTRATO_ATIVO,
        prioridade: ALERTA_PRIORIDADE.INFO,
        dedupeKey: `${ALERTA_TIPO.PARCEIRO_SEM_CONTRATO_ATIVO}:${lugar.id}`,
        titulo: `"${lugar.nome}" — parceiro sem contrato`,
        mensagem: "Parceiro ativo no app sem registro comercial ativo em Contratos.",
        href: "/admin/contratos",
        createdAt: new Date().toISOString(),
        icon: ALERTA_ICONE[ALERTA_TIPO.PARCEIRO_SEM_CONTRATO_ATIVO],
        devOnly: true,
      });
    }
