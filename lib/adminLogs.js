...
export const LOG_ACOES = [
  ...
  { id: "visualizou_lugar", label: "Visualização", values: ["visualizou_lugar"] },
  { id: "escaneou_qr", label: "QR Code", values: ["escaneou_qr"] },
  {
    id: "claim_perfil",
...
/**
 * Conta total de registros na tabela `logs` (admin).
 * @param {SupabaseClient} supabase
 * @returns {Promise<number>}
 */
export async function fetchTotalLogsCount(supabase) {
  const { count, error } = await supabase
    .from("logs")
    .select("id", { count: "exact", head: true });

  if (error) {
    console.error("[fetchTotalLogsCount]", error.message);
    return 0;
  }
...
/**
 * Remove todos os logs (ação irreversível; requer policy `logs_delete_admin`).
 * @param {SupabaseClient} supabase
 * @returns {Promise<{ error: import('@supabase/supabase-js').PostgrestError | null }>}
 */
export async function clearAllAdminLogs(supabase) {
