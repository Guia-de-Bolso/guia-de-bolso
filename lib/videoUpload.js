...
/**
 * Valida arquivo pronto para upload (após otimização).
 * @param {File} file
 * @returns {Promise<{ durationSeconds: number, width: number, height: number }>}
 */
export async function validateVideoOutputFile(file) {
  if (!isAcceptedVideoFile(file)) {
    throw new Error("Formato inválido após otimização.");
  }

  if (file.size > LUGAR_VIDEO_LIMITS.maxOutputBytes) {
    throw new Error(
      `Vídeo otimizado ainda é grande (${formatVideoFileSize(file.size)}). Máximo ${formatVideoFileSize(LUGAR_VIDEO_LIMITS.maxOutputBytes)}.`
    );
  }
