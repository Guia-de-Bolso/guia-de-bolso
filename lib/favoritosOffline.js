...
export const FAVORITO_OFFLINE_REMOVED_MESSAGE = "Removido dos favoritos.";
...
export async function listOfflineFavoritos(userId) {
  ...
  const lugares = [];
  const atrativos = [];
  let metaSyncedAt = null;
  let latestItemSavedAt = null;
  ...
  for (const row of rows) {
    ...
    if (row.key === buildOfflineMetaKey(userId) && row.payload?.lastSyncedAt) {
      metaSyncedAt = row.payload.lastSyncedAt;
    }
    if (row.savedAt && (row.type === FAVORITO_OFFLINE_TYPES.LUGAR || row.type === FAVORITO_OFFLINE_TYPES.ATIVO)) {
      if (!latestItemSavedAt || row.savedAt > latestItemSavedAt) {
        latestItemSavedAt = row.savedAt;
      }
    }
  }

  const lastSyncedAt =
    metaSyncedAt && latestItemSavedAt
      ? metaSyncedAt > latestItemSavedAt
        ? metaSyncedAt
        : latestItemSavedAt
      : metaSyncedAt || latestItemSavedAt;
