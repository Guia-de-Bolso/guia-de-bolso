 ...
 * @property {KpiTopLugar[]} topLugaresComerciais
 * @property {KpiTopLugar[]} topLugaresCuradoria
 */
...
/**
 * Local com potencial de receita (pode virar plano pago).
 * Exclui Natureza/público e conteúdo curado pelo guia (nunca cobrado).
 * @param {object|null|undefined} lugar
 * @returns {boolean}
 */
export function isLugarPotencialReceita(lugar) {
  if (!lugar) return false;
  if (isConteudoCuradoria(lugar)) return false;
 ...
 * @returns {KpiTopLugar[]}
 */
export function buildTopLugaresFromLogs(logs, lugaresById, limit = 10) {
...
/**
 * Top engajamento separado: potencial de receita vs curadoria/público (nunca pago).
 * @param {Array<{ acao: string, detalhes: object|null }>} logs
 * @param {Map<string, object>} lugaresById
 * @param {number} [limit=10]
 * @returns {{ comerciais: KpiTopLugar[], curadoria: KpiTopLugar[] }}
 */
export function buildTopLugaresPorReceita(logs, lugaresById, limit = 10) {
  const ranked = buildTopLugaresFromLogs(logs, lugaresById, 0);
  /** @type {KpiTopLugar[]} */
  const comerciais = [];
  /** @type {KpiTopLugar[]} */
  const curadoria = [];
  ...
  for (const item of ranked) {
    const lugar = lugaresById.get(item.id);
    if (isLugarPotencialReceita(lugar)) comerciais.push(item);
    else curadoria.push(item);
  }

  return {
    comerciais: comerciais.slice(0, limit),
    curadoria: curadoria.slice(0, limit),
  };
}
...
export async function buildKpisLancamento(supabase, periodId) {
  ...
  const [
    ...
    supabase
      .from("lugares")
      .select(
        "id, nome, categoria, subcategoria, status, eh_parceiro, perfil_promo_ate, conteudo_curadoria"
      ),
    supabase
      .from("logs")
  ...
  const { comerciais: topLugaresComerciais, curadoria: topLugaresCuradoria } =
    buildTopLugaresPorReceita(logsEngajamentoRes.data ?? [], lugaresById);
