import { getStatusFuncionamento, horariosTemCadastro } from "./horarios.js";
...
/**
 * Indica se o lugar exibe horário comercial cadastrado (estabelecimentos).
 * Atrativos públicos (praias, trilhas, lagoas) costumam ter `mostrar_horarios` desligado.
 * @param {{ horarios?: Record<string, string>, mostrar_horarios?: boolean }} [lugar]
 * @returns {boolean}
 */
export function lugarTemHorarioComercial(lugar) {
 ...
 * Locais sem horário comercial não entram no filtro "abertos agora".
 * @param {{ horarios?: Record<string, string>, mostrar_horarios?: boolean }} [lugar]
 * @returns {{ abertoAgora: boolean, statusLabel: string, statusDetail: string }}
 */
function getStatusBusca(lugar) {
  if (!lugarTemHorarioComercial(lugar)) {
    return {
      abertoAgora: false,
  ...
  const status = getStatusFuncionamento(lugar.horarios, true);
  if (!status) {
    return {
      abertoAgora: false,
      statusLabel: "Fechado",
...
export function filtrarLugaresPorStatus(lugares, filtroStatus) {
  ...
  if (filtroStatus === FILTRO_STATUS_BUSCA.ABERTOS) {
    return (lugares ?? []).filter(
      (lugar) => lugarTemHorarioComercial(lugar) && lugarEstaAberto(lugar)
    );
  }
