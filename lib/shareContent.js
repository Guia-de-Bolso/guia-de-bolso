...
/**
 * URL canônica de roteiro curado (marketing host).
 * @param {string} rotaId
 * @returns {string}
 */
export function getRoteiroShareUrl(rotaId) {
  const id = String(rotaId || "").trim();
  return buildSharePublicUrl(roteiroDetalhePath(id));
}

/** @deprecated Prefer getRoteiroShareUrl — mantido para callers legados. */
export function getAtrativoShareUrl(rotaId) {
  return getRoteiroShareUrl(rotaId);
}
...
async function copyShareUrl(shareUrl) {
  if (typeof navigator !== "undefined" && navigator.clipboard?.writeText) {
    try {
      ...
      // WebViews nativos às vezes bloqueiam a Clipboard API.
    }
  }
