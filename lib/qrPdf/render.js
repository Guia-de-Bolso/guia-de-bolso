...
function drawStickerBackground(doc, pageW, pageH) {
  ...
  doc.circle(pageW * 0.93, pageH * 0.9, pageW * 0.045, "F");
  doc.circle(pageW * 0.08, pageH * 0.11, pageW * 0.03, "F");
...
function drawBrandHeader(doc, cx, y, logoDataUrl) {
  ...
  if (logoDataUrl) {
    ...
    const logoY = y + (pillH - logoH) / 2;
    doc.addImage(logoDataUrl, "PNG", logoX, logoY, logoW, logoH);
    textX = logoX + logoW + logoGap + textW / 2;
  }

  doc.setTextColor(255, 255, 255);
  doc.text(label, textX, y + pillH * 0.68, { align: "center", charSpace });
  return pillH;
}
