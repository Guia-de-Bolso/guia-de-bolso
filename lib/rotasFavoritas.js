 ...
 * @returns {Promise<boolean|null>} null = tabela ausente; true = ok; false = erro de rede/dados
 */
export async function toggleRotasFavorita(
  ...
  if (toggleLocks.has(key)) return true;
  ...
  try {
    ...
    void cacheAtrativoFavoritoFromServer(supabase, user.id, String(rotaId));
