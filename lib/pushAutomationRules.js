...
/** Frase no corpo da push — a categoria nunca entra como sujeito da oração. */
const NOVO_LOCAL_INTRO_POR_CATEGORIA = Object.freeze({
  Natureza: "Um novo ponto de natureza",
  Gastronomia: "Um novo lugar para comer",
  Noite: "Um novo ponto na noite",
  Serviços: "Um novo serviço",
  Cultura: "Um novo ponto de cultura",
  Aventura: "Um novo ponto de aventura",
  "Bem-estar": "Um novo espaço de bem-estar",
});
 ...
 * @returns {string}
 */
export function novoLocalPushIntro(categoria) {
  const key = String(categoria ?? "").trim();
  return NOVO_LOCAL_INTRO_POR_CATEGORIA[key] || "Um novo lugar";
}

/**
 * Copy da campanha quando um local vira público. Espelhada no trigger
 * `enqueue_lugar_push_campaigns` (migration 20260901120000).
 * @param {object} lugar
 * @returns {object}
 */
export function buildNovoLocalCampaign(lugar) {
  const nome = String(lugar?.nome ?? "").trim() || "Um novo lugar";
  return {
    event_key: `novo_local:${lugar.id}`,
    type: PUSH_CAMPAIGN_TYPES.NOVO_LOCAL,
    audience: "all",
    title: `Novo no Guia: ${nome}`,
    body: `${novoLocalPushIntro(lugar.categoria)} acabou de chegar ao Guia de Bolso. Conheça agora.`,
 ...
 * @returns {object}
 */
export function buildNovoParceiroCampaign(lugar) {
  const nome = String(lugar?.nome ?? "").trim() || "Um estabelecimento";
  return {
    event_key: `novo_parceiro:${lugar.id}`,
    type: PUSH_CAMPAIGN_TYPES.NOVO_PARCEIRO,
    audience: "all",
    title: "🤝 Novo Parceiro Oficial",
    body: `${nome} agora é Parceiro Oficial. Abra o app e conheça.`,
    url: `/lugares/${lugar.id}`,
  };
}
...
export function buildWeatherCampaign(clima, dateISO) {
  ...
  const condition = String(clima.condition || "tempo bom").toLowerCase();
  ...
  return {
    ...
    audience: "all",
    title: `${emoji} Tempo bom em Imbituba`,
    body: `Hoje tá ${condition}, com máxima de ${max}°C. Abra o app e escolha o passeio.`,
    url: "/",
  };
}
...
export function buildWeeklyHighlightCampaign(lugar, date = new Date()) {
  ...
  return {
    ...
    body: `Esta semana o destaque é ${lugar.nome}. Toque e confira no Guia de Bolso.`,
    url: `/lugares/${lugar.id}`,
  };
}
...
export function buildRoteiroReminderCampaign(roteiro) {
  return {
    ...
    title: "🧭 Seu roteiro te espera",
    body: `Lembra do “${roteiro.titulo}”? Abra o app e retome o planejamento.`,
    url: "/roteiros",
  };
}
