...
export const CONTRATO_TIPO_OPTIONS = [
  { id: CONTRATO_TIPO.LANCAMENTO_6_MESES, label: "Lançamento — 6 meses grátis" },
...
export const CONTRATO_FILTROS = [
  ...
  { id: "pago", label: "Pagos" },
  { id: "vencendo", label: "Grátis vencendo (30d)" },
...
/**
 * Parceiro ativo no app sem contrato comercial ativo registrado.
 * @param {object} lugar
 * @param {Record<string, object>} contratosAtivosPorLugar
 * @returns {boolean}
 */
export function deveAlertarParceiroSemContratoAtivo(lugar, contratosAtivosPorLugar = {}) {
  if (!lugar?.eh_parceiro) return false;
  return !contratosAtivosPorLugar[lugar.id];
}
...
/**
 * Estabelecimentos elegíveis no dropdown de contratos (somente parceiros ativos).
 * @param {object|null|undefined} lugar
 * @returns {boolean}
 */
export function isLugarElegivelParaContratoDropdown(lugar) {
  if (!lugar) return false;
  return Boolean(lugar.eh_parceiro) && lugar.status === "ativo";
}
...
export async function fetchLugaresParaContrato(supabase) {
  const { data, error } = await supabase
    .from("lugares")
    .select("id, nome, categoria, status, eh_parceiro")
    .eq("eh_parceiro", true)
    .eq("status", "ativo")
    .order("nome");
