...
export const ANDROID_VOICE_POPUP_TIMEOUT_MS = 90000;

/** Tempo máximo para preparar permissões / reset antes do popup. */
export const ANDROID_VOICE_PREPARE_TIMEOUT_MS = 8000;
...
export const VOICE_SEARCH_MESSAGES = {
  ...
  POPUP_TIMEOUT:
    "O diálogo de voz demorou demais. Instale/atualize o app Google e tente de novo.",
  POPUP_UNAVAILABLE:
    "Nenhum app de voz encontrado. Instale ou atualize o app Google no celular.",
  NATIVE_VOICE_BUILD_REQUIRED:
    "Busca por voz precisa do app 1.0.14+. Reinstale pelo Android Studio (Run ▶) ou aguarde a Play Store.",
};
...
export function formatSpeechError(error) {
  ...
  if (/GuiaVoiceSearch.*not implemented|plugin-timeout/i.test(message)) {
    return VOICE_SEARCH_MESSAGES.NATIVE_VOICE_BUILD_REQUIRED;
  }
  ...
  if (/NO_SPEECH_HANDLER/i.test(message)) {
    return VOICE_SEARCH_MESSAGES.POPUP_UNAVAILABLE;
  }

  if (/voice-idle-timeout|voice-prepare-timeout|popup-timeout/i.test(message)) {
    return VOICE_SEARCH_MESSAGES.POPUP_TIMEOUT;
  }
...
export async function ensureVoiceSessionIdle() {
  ...
  try {
    await withVoiceCaptureTimeout(resetNativeSpeechBridge(), 4000, "voice-idle-timeout");
  } catch {
    /* noop */
  }
}
...
/**
 * Para listeners/sessões presas sem bloquear a UI.
 * @returns {Promise<void>}
 */
export async function resetNativeSpeechBridge() {
  try {
    const SpeechRecognition = await loadSpeechRecognitionPlugin();
    await Promise.all([
      SpeechRecognition.stop().catch(() => undefined),
      SpeechRecognition.forceStop({ timeout: 800 }).catch(() => undefined),
    ]);
    await SpeechRecognition.removeAllListeners().catch(() => undefined);
  } catch {
    /* noop */
...
/**
 * Verifica se o plugin GuiaVoiceSearch está no APK instalado.
 * @returns {Promise<void>}
 */
export async function assertGuiaVoiceSearchPlugin() {
  const { GuiaVoiceSearch } = await import("./guiaVoiceSearch.js");
  await withVoiceCaptureTimeout(GuiaVoiceSearch.ping(), 3000, "plugin-timeout");
}
...
export async function runAndroidPopupVoiceCapture() {
  ...
  if (!permission.granted) {
    throw new Error(permission.error ?? VOICE_SEARCH_MESSAGES.PERMISSION_DENIED);
  }

  try {
    await assertGuiaVoiceSearchPlugin();
  } catch {
    throw new Error(VOICE_SEARCH_MESSAGES.NATIVE_VOICE_BUILD_REQUIRED);
  }

  const language = await resolveSpeechLocaleForAndroid();
  const prompt = "O que você quer descobrir em Imbituba?";
  const { GuiaVoiceSearch } = await import("./guiaVoiceSearch.js");

  const result = await withVoiceCaptureTimeout(
    GuiaVoiceSearch.speak({ language, prompt }),
 ...
 * @returns {Promise<string>}
 */
async function resolveSpeechLocaleForAndroid() {
  try {
    const SpeechRecognition = await loadSpeechRecognitionPlugin();
    return resolveSpeechLocale(SpeechRecognition);
  } catch {
    return SPEECH_LANG;
  }
}
...
async function assertNativeSpeechPlugin(SpeechRecognition) {
  try {
    const { version } = await withVoiceCaptureTimeout(
      SpeechRecognition.getPluginVersion(),
      3000,
...
export async function ensureSpeechPermissions() {
  ...
  try {
    ...
    const status = await withVoiceCaptureTimeout(
    ...
    const requested = await withVoiceCaptureTimeout(
